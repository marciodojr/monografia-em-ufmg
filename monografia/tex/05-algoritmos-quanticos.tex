...
Neste capítulo será feito o estudo de dois algoritmos quânticos, o algoritmo de Grover e o Algoritmo Quântico de Otimização Aproximada (QAOA). O algoritmo de Grover é um algoritmo para busca não estruturada em um conjunto de dados que utiliza uma função oráculo para avaliar o valor (ou valores) procurados, publicado em 1996 \cite{groveralgo}. A vantagem deste algoritmo é que é exigido somente $O(\sqrt{N})$ avaliações do oráculo para encontrar os valores, onde $N$ é a cardinalidade do domínio da busca. A busca não estruturada clássica possui complexidade $O(N)$, o algoritmo de Grover oferece um ganho bastante expressivo em relação aos algoritmos clássicos. O segundo algoritmo, QAOA, publicado em \cite{farhi2014quantumapproximateoptimizationalgorithm}, é um algoritmo quântico heurístico utilizado em problemas de otimização combinatória com o objetivo de encontrar soluções aproximadas, cujas aproximações podem ser aumentadas com o aumento do número de camadas do circuito, embora tal melhoria não seja garantida para valores finitos de camadas do circuito. O QAOA é de grande interesse pois oferece uma alternativa quântica aos algoritmos clássicos para busca de soluções aproximadas em problemas combinatórios. Alguns exemplos de problemas que podem ser mapeados como problemas de otimização combinatória e pertencem à classe \textsf{NP-HARD} são o Problema do Caixeiro Viajante, Problema da Mochila, Coloração de Grafos, Corte Máximo em grafos entre outros. Neste capítulo, o QAOA será aplicado para buscar de corte máximo aproximado em grafos.
...
A implementação do operador $U_f$ depende da forma como a função booleana $f$ é calculada. Em particular, assume-se que existe um circuito quântico que calcula $f(x)$ com custo polinomial no número $n$ de bits da entrada. A partir desse circuito, pode-se construir $U$ dado pela equação \eqref{eq:oracle}. Uma técnica utilizada para calcular $U_f\ket{x}$ é chamada de \textit{phase kickback}, calcula-se $f(x)$ em um qubit auxiliar preparado em um autoestado apropriado da porta $X$ e, em seguida, desfaz-se a computação auxiliar, permanecendo apenas a fase $(-1)^{f(x)}$ associada ao estado $\ket{x}$. Assim, se $f$ admite uma implementação por um circuito de tamanho polinomial em $n$, então o operador $U_f$ também pode ser implementado com custo polinomial em $n$. É importante observar que o algoritmo de Grover mede sua complexidade principalmente pelo número de consultas ao oráculo; o custo interno de cada consulta depende da aplicação considerada \cite[Seção~6.1.2]{nielsen2010quantum}. O exemplo \ref{ex:opkb} mostra a implementação de $U_f$ para $f(x_1,x_2)=x_1\wedge x_2$ utilizando o \text{phase kickback}.

\begin{example}[Implementação de um oráculo por \textit{phase kickback}]\label{ex:opkb}
Considere a função booleana $\mathrm{AND}$ dada por
\begin{equation}
    f:\{0,1\}^2\longrightarrow\{0,1\},
    \qquad
    f(x_0,x_1)=x_0 \wedge x_1,
\end{equation}
isto é, $f(x_0,x_1)=1$ se, e somente se, $x_0=x_1=1$. Para implementar o operador
Para implementar o operador
$$
    U_f\ket{x}=(-1)^{f(x)}\ket{x},
$$
introduzimos um qubit auxiliar preparado no estado
$$
    \ket{-}
    =
    \frac{\ket{0}-\ket{1}}{\sqrt{2}}.
$$
A função $f$ pode ser implementada utilizando a porta
Toffoli, denotada por $\mathrm{CCX}$, com os qubits $x_0$ e $x_1$
como controles e o qubit auxiliar como alvo. Sua ação é dada por
$$
    \mathrm{CCX}\ket{x_0x_1y}
    =
    \ket{x_0x_1\left(y\oplus x_0x_1\right)}.
$$
Aplicando a porta ao estado
$$
    \ket{x_0x_1}\ket{-},
$$
temos dois casos. Se $x_0x_1\neq 11$, pelo menos um dos qubits de
controle é igual a zero e a porta $\mathrm{CCX}$ não altera o qubit
auxiliar. Portanto,
$$
    \mathrm{CCX}\ket{x_0x_1}\ket{-}
    =
    \ket{x_0x_1}\ket{-}.
$$
Por outro lado, se $x_0=x_1=1$, a porta $\mathrm{CCX}$ aplica o
operador $X$ ao qubit auxiliar. Como
$$
    X\ket{-}=-\ket{-},
$$
segue que
$$
    \mathrm{CCX}\ket{11}\ket{-}
    =
    \ket{11}X\ket{-}
    =
    -\ket{11}\ket{-}.
...
$$
\begin{aligned}
    \mathrm{CCX}\ket{00}\ket{-}
        &= \ket{00}\ket{-},\\
    \mathrm{CCX}\ket{01}\ket{-}
        &= \ket{01}\ket{-},\\
    \mathrm{CCX}\ket{10}\ket{-}
        &= \ket{10}\ket{-},\\
    \mathrm{CCX}\ket{11}\ket{-}
        &= -\ket{11}\ket{-}.
\end{aligned}
$$
Como $f(x)=0$ para $x\in\{00,01,10\}$ e $f(11)=1$, podemos escrever
o resultado de forma compacta como
$$
    \mathrm{CCX}\ket{x}\ket{-}
    =
    (-1)^{f(x)}\ket{x}\ket{-}.
$$
Portanto, desconsiderando o qubit auxiliar, a porta Toffoli implementa,
por \textit{phase kickback}, o operador
$$
    U_f\ket{x}=(-1)^{f(x)}\ket{x},
$$
invertendo a fase apenas do estado $\ket{11}$.
\end{example}
...
assim,
\begin{equation}
    \ket{s} = \frac{1}{\sqrt{N}}\sum_{x\in \{0,1\}^n}\ket{x} \implies \ket{s} = \sqrt{\frac{M}{N}}\ket{x^\star} + \sqrt{\frac{N-M}{N}}\ket{x^\star_\perp}.
\end{equation}
...
\end{equation}
Por outro lado, note que o valor máximo absoluto de $M_p$ é dado pelo lema \ref{lm:pvoc}.
...
No QAOA, os estados permitidos são restritos aos estados unitários na forma $\ket{\gamma, \beta}$\footnote{A determinação dos parâmetros $\boldsymbol{\gamma}$ e $\boldsymbol{\beta}$ constitui uma etapa importante do QAOA. Além de métodos clássicos de otimização, diferentes estratégias para essa escolha têm sido propostas. Entre elas encontram-se métodos de aprendizado por reforço, nos quais a escolha dos parâmetros é formulada como um problema de aprendizado de uma política \cite{pmlr-v107-yao20a}, redes neurais treinadas para prever boas inicializações dos parâmetros \cite{Xie_2023}, otimização bayesiana e estratégias baseadas na transferência de parâmetros previamente otimizados entre diferentes instâncias do problema \cite{galda2021transferabilityoptimalqaoaparameters}. Tais abordagens buscam reduzir o custo da otimização clássica dos $2p$ parâmetros ou fornecer pontos iniciais favoráveis para essa otimização. Em geral, entretanto, esses
procedimentos não fornecem uma garantia de que o máximo global de $F_p$ seja encontrado, mantendo-se o caráter heurístico da escolha dos parâmetros do QAOA.} conforme equação \eqref{eq:gammabeta}, assim, não necessariamente temos
\begin{equation}
...
Seja $G=(V,E)$ um grafo não direcionado, com um conjunto $V$ de vértices e $E$ de arestas. Um corte de $G$ é uma partição do conjunto de vértices em dois subconjuntos. Por exemplo, podemos escolher $A\subseteq V$ e tomar $\bar{A}=V\setminus A$, assim, um corte de $G$ pode ser visto como a escolha de um subconjunto $A$ de $V$. O valor do corte é o número de arestas que tem uma extremidade em $A$ e outra fora de $A$,
\begin{equation}
...
Seja $\text{Cut}(A) \subseteq V$ um corte de um grafo como visto na seção~\ref{sec:cm}. Associamos a cada vértice $i\in V$ uma variável binária
\begin{equation}
    z_i\in \{0, 1\}, \qquad z_i = 1 \iff i \in \text{Cut}(A).
\end{equation}
Assim, uma aresta $\{i,j\}$ contribui para o corte se e somente se $z_i\neq z_j$. Sendo $\oplus$ a operação de adição modulo 2, para $1\leq i,j \leq n$, definimos a função $C_{ij}: \{0,1\}^n\to \{0,1\}$ como
\begin{equation}
