...
\section{Funções de operadores}
\label{sec:funcoes_operadores}

Em diversas situações da mecânica quântica e da computação quântica, é necessário definir funções cujo argumento é um operador linear, tipicamente um operador hermitiano ou unitário. Exemplos fundamentais incluem a exponencial de operadores, utilizada na evolução temporal $e^{-iHt}$ e funções associadas a hamiltonianos de custo no QAOA.

Nesta seção apresentamos a definição de funções de operadores em dimensão finita, com ênfase no caso hermitiano.

\subsection{Decomposição espectral}

Quando $A$ é um operador normal (em particular, hermitiano), podemos utilizar sua decomposição espectral para definir funções de operadores.

\begin{theorem}[Teorema espectral]
Seja $A:\mathcal{H}\to \mathcal{H}$ um operador hermitiano em um espaço de Hilbert finito. Então existe uma base ortonormal $\{\ket{\psi_j}_{j=1}^n\}$ de autovetores de $A$ tal que
\begin{equation}
    A = \sum_{j}^n \lambda_j \ket{\psi_j}\bra{\psi_j},
\end{equation}
onde $\lambda_j \in \mathbb{R}$ são os autovalores de $A$.
\end{theorem}
Para qualquer função $f : \mathbb{R} \to \mathbb{C}$ define-se
\begin{equation}
    f(A) = \sum_{j} f(\lambda_j)\ket{\psi_j}\bra{\psi_j}.
\end{equation}
Observe que a função atua apenas sobre os autovalores, preservando os autovetores.

\begin{example}[Exponencial de operador]\label{ex:opex}
	A função mais importante em mecânica quântica é a exponencial. Se $H$ é um operador hermitiano, define-se
	\begin{equation}
		e^{-iHt} = \sum_{k=0}^{\infty} \frac{(-it)^k}{k!} H^k.
	\end{equation}

	Pela decomposição espectral de $H = \sum_j \lambda_j \ket{v_j}\bra{v_j}$, obtemos
	\begin{equation}
		e^{-iHt}
		=
		\sum_j e^{-i\lambda_j t} \ket{v_j}\bra{v_j}.
	\end{equation}

	Segue imediatamente que $e^{-iHt}$ é unitário, pois
	\begin{equation}
		e^{-iHt} \left(e^{-iHt}\right)^\dagger
		=
		\sum_j |e^{-i\lambda_j t}|^2 \ket{v_j}\bra{v_j}
		=
		I.
	\end{equation}
\end{example}
...
\end{equation}
Dado o espaço de Hilbert $\mathcal{H}$ podemos construir o espaço $\mathcal{H}^{\otimes^n} = \bigotimes_{i=1}^n\mathcal{H}$. Sendo $A: \mathcal{H}\to \mathcal{H}$ um operador, denotamos por $A^{\otimes^n}: \mathcal{H}^{\otimes^n}\to \mathcal{H}^{\otimes^n}$ o operador linear
\begin{equation}
